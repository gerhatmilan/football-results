...
try:
    client = APIClient(API_CONFIG_FILE, COUNTRIES_CONFIG_FILE)
    updater = CountriesUpdater(DATABASE_CONFIG_FILE, COUNTRIES_CONFIG_FILE)
    
    data = get_data(client=client, mode=MODE, save=True, config=COUNTRIES_CONFIG_FILE)
