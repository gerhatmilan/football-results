...
try:
    client = APIClient(API_CONFIG_FILE,updaters.TEAMS_CONFIG_FILE)
    ...
    for league_id in included_league_ids:
        ...
        for season in available_seasons:
            try:
                data = get_data(client=client, mode=MODE, save=True, config=updaters.TEAMS_CONFIG_FILE, filename_parameters=(league_id, season), endpoint_parameters=(league_id, season))
            ...
            updater = TeamsUpdater(DATABASE_CONFIG_FILE, updaters.TEAMS_CONFIG_FILE)
            updater.update(data)

            logging.log(SUCCESS_LOG_PATH, f'Update of table teams has completed successfully for league {league_id} and season {season}, total changes: {updater.total_changes}\n')
